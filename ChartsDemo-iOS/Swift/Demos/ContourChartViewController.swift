//
//  ContourChartViewController.swift
...
class ContourChartViewController: DemoBaseViewController, ContourChartViewDelegate {
            ...
            coordinator.animate(alongsideTransition: { (context) in
                if let _chartView = self.chartView,
                   ...
                   let oldContour = _dataSets.first(where: { $0.identifier == (self.currentContour?.functionExpression == nil ? "data" : "function") } ) {   // should only be one contour set per chart!!
                    ...
                    if ratio > 1 {
                        let lengthRatio = (self.maxY - self.minY) / (self.maxX - self.minX)
                        yRange = Range(from: self.minY, to: self.maxY)
                        xRange = Range(from: self.minX * ratio * lengthRatio * 0.25, to: self.maxX * ratio * lengthRatio * 0.75)
                    }
                    else {
                        let lengthRatio = (self.maxX - self.minX) / (self.maxY - self.minY)
                        xRange = Range(from: self.minX, to: self.maxX)
                        yRange = Range(from: self.minY / ratio / lengthRatio * 0.25, to: self.maxY / ratio / lengthRatio * 0.75)
                    }
                    
                    // Axes
                    _chartView.leftAxis.axisMinimum = yRange.minLimit
                    _chartView.leftAxis.axisMaximum = yRange.maxLimit
                    
                    _chartView.rightAxis.axisMinimum = yRange.minLimit
                    _chartView.rightAxis.axisMaximum = yRange.maxLimit
                   
                    _chartView.xAxis.axisMinimum = xRange.minLimit
                    _chartView.xAxis.axisMaximum = xRange.maxLimit
                    
                    self.showChart(oldContour, renewal: true)
                }
            }) { (context) in
...
//                }
    ...
    override func viewDidLoad() {
        ...
        contourManagerRecords.append(ContourManagerRecord(fillContours: true, extrapolateToARectangleOfLimits: true, krigingSurfaceInterpolation: true, krigingSurfaceModel: .exponential, trig: false, functionLimits: [500, 500], plottitle: "Barametric Contours", functionExpression: nil, data:[
        ...
        self.options = [.animateX,
                        ...
                        .toggleData,
                        .toggleContourNoIsoCurves,
                        .toggleContourInterpolationMethod,
                        .toggleGridPrimaryResolution,
                        .toggleGridSecondaryResolution,
                        .toggleExtrapolateToCorners,
                        .toggleSurfaceInterpolation]
        ...
        if let _chartView = self.chartView,
           ...
           let yAxisValueFormatter = _chartView.leftAxis.valueFormatter {
            let marker = ContourMarkerView(color: UIColor(white: 180/250, alpha: 1),
    ...
    private func updateLegend() {
        if let l = self.chartView?.legend {
            l.resetCustom()
            l.font = NSUIFont.systemFont(ofSize: 14, weight: .light)
            l.xOffset = 25
                    ...
                    if let _fillings = dataSet.isoCurvesFillings {
                        ...
                        for i in 0..<_fillings.count {
                            ...
                            if let fill = filling.fill as? ColorFill {
                                legendEntry.formColor = NSUIColor(cgColor: fill.color)
                            }
        ...
        if let _chartView = self.chartView,
           let _data = _chartView.data {
            ...
            case .toggleHighlightCircle:
                for case let set as ContourChartDataSet in _data {
            ...
            case .toggleContourInterpolationMethod:
                changeInterpolationMethod()
                
            case .toggleGridPrimaryResolution:
                changePrimaryResolution()
                
            case .toggleGridSecondaryResolution:
                changeSecondaryResolution()
                
            case .toggleExtrapolateToCorners:
                changeExtrapolateToCorners()
                
            case .toggleSurfaceInterpolation:
                changeSurfaceInterpolationMethod()
        ...
        if let _chartView = self.chartView,
            ...
            if let _chartView = self.chartView,
               ...
               let yAxisValueFormatter = _chartView.leftAxis.valueFormatter {
                ...
                let dataSet = ContourChartDataSet(label: _currentContour.plottitle)
                let data: ContourChartData = [ dataSet ]
                data.setValueFont(.systemFont(ofSize: UIDevice.current.userInterfaceIdiom == .phone ? 10.0 : 14.0, weight: .light))
                _chartView.data = data
                
                if var set = _chartView.contourData?.dataSets.first as? ContourChartDataSet {
                    ...
                    set.catmullCustomAlpha = 0.7
                    
                    set.alignsPointsToPixels = true
                    set.noIsoCurves = noIsoCurves
                    set.functionPlot = _currentContour.functionExpression != nil
                    set.minFunctionValue = minFunctionValue
                    set.maxFunctionValue = maxFunctionValue
                    set.limits = contourLimits
                    set.extrapolateToLimits = _currentContour.extrapolateToARectangleOfLimits
                    set.fillIsoCurves = _currentContour.fillContours
                    set.easyOnTheEye = true
                    
                    set.isoCurvesLineWidth = 3
                    set.isoCurvesLineColour = .blue
                    
                    set.isoCurvesLabelFont = UIFont.systemFont(ofSize: UIDevice.current.userInterfaceIdiom == .phone ? 10.0 : 14.0, weight: .light)
                    set.isoCurvesLabelTextColor = .black
                    let labelFormatter = NumberFormatter()
                    labelFormatter.maximumFractionDigits = 2
                    set.isoCurvesLabelFormatter = labelFormatter
                    
                    if let identifier = set.identifier,
                       identifier == "data" {
                        currentContour?.functionLimits = [ minX , maxX, minY, maxY ]
                        set.limits = [ minX , maxX, minY, maxY ]
                        ...
                        let values = self.plotdata.map { ChartDataEntry(x: $0.x , y: $0.y, data: $0.z) }
                        set.replaceEntries(values)
                        set.fieldBlock = { xValue, yValue in
...
//            }
    ...
    private func changeContour() {
        var noIsoCurves: Int = 6
        if let _chartView = self.chartView {
            if let _contourData = _chartView.contourData,
               var _dataSets = _contourData.dataSets as? [ContourChartDataSet],
               let oldContour = _dataSets.first(where: { $0.identifier == (self.currentContour?.functionExpression == nil ? "data" : "function") } ) {   // should only be one contour set per chart!!
                ...
                if let newContour = setupContour(noIsoCurves: noIsoCurves) {
                    _dataSets.remove(object: oldContour)
                    if let l = chartView?.legend {
                        l.resetCustom()
                    }
        ...
        if let _currentContour = self.currentContour {
            if let _ /*spinner*/ = self.spinner {
                ...
                if _currentContour.functionExpression == nil {
                    ...
                    newContour.setContourShape(.square)
                }
                else {
    ...
    private func changeNoOfIsoCurves() {
        if let _chartView = self.chartView {
            if let _contourData = _chartView.contourData,
               let _dataSets = _contourData.dataSets as? [ContourChartDataSet],
    ...
    private func changePrimaryResolution() {
        if let _chartView = self.chartView,
           var _currentContour = self.currentContour {
            ...
            if var index = resolutions.firstIndex(where: { $0 == _currentContour.firstResolution }) {
                ...
                if let set = _chartView.contourData?.dataSets.first as? ContourChartDataSet {
                    set.noRowsFirst = _currentContour.firstResolution
                    set.noColumnsFirst = _currentContour.firstResolution
                    if let _ = self.spinner {
                        showSpinner() { _ in
    ...
    private func changeSecondaryResolution() {
        if let _chartView = self.chartView,
           var _currentContour = self.currentContour {
    ...
    private func changeInterpolationMethod() {
        if let _chartView = self.chartView,
           let set = _chartView.contourData?.dataSets.first as? ContourChartDataSet {
    ...
    private func changeExtrapolateToCorners() {
        if let _chartView = self.chartView,
    ...
    private func changeSurfaceInterpolationMethod() {
        if let _chartView = self.chartView,
