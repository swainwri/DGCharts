        ...
        if let _majorAxis = self.majorAxis,
           _majorAxis.drawGridLinesEnabled {
            ...
            if self.axis.gridLinesToChartRectEdges {
                ...
                let pixelPointAxisMaximim: CGPoint = _transformer.pixelForValues(x: 0, y: axis.axisMaximum)
                let cornerEntry: CGFloat = centreToCorner / pixelPointAxisMaximim.y * 2 * axis.axisMaximum * 1.1
            ...
            for j in 0 ..< labelCount {
                if adjustedEntries[j] > 0 {  // don't need negative entries as psoitive one with take care of concentric circles
